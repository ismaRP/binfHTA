...
ggplot(chip) + geom_histogram(aes(x=score)) + facet_wrap(~chr) # plot distribution of scores across each chromosome

#Based on our plot we assume that the score values follows a normaldistributin.

chip_clean <- filter(chip, score != "NA") # remove NA results

oneway.test(score ~ as.factor(chr), data=chip) #Anova test for significance difference between score values across chromosomes

#p-value is larger than 0.05, therfore we cannot reject our null hypothesis. 

pairwise.t.test(chip_clean$score, g=chip_clean$chr)

new_chip<-mutate(chip, site.width = end - start) #create site.width (end - start)
head(new_chip) #new data
ggplot(new_chip) + geom_histogram(aes(x=site.width)) + facet_wrap(~chr) # plot distribution of site width across each chromosome
...
#p-value below 0.05 threshold, therefore we can reject null hypothesis
```
