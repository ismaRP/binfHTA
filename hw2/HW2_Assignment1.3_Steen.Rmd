...
nice bedtools genomecov -i sorted_ERa_hg18.bed -g hg18_chrom_sizes.txt -max 1 > ERa_genomecoverage.bed
nice bedtools genomecov -i sorted_ERb_hg18.bed -g hg18_chrom_sizes.txt -max 1 > ERb_genomecoverage.bed
...
library(tidyverse) #activate tidyverse package

ERa_genome_coverage <- as.data.frame(read.table("ERa_genomecoverage.bed",header = FALSE, sep="\t",stringsAsFactors=FALSE, quote="")) #Read data
colnames(ERa_genome_coverage) <- c("Chromosome","Depth_of_Coverage","Nr_of_Bases","Size_chr","Fraction_of_Coverage") #add column names
ERa_genome_coverage


ERb_genome_coverage <- as.data.frame(read.table("ERb_genomecoverage.bed",header = FALSE, sep="\t",stringsAsFactors=FALSE, quote="")) #Read data
colnames(ERb_genome_coverage) <- c("Chromosome","Depth_of_Coverage","Nr_of_Bases","Size_chr","Fraction_of_Coverage") #add column names
ERa_genome_coverage
...
# both tibbles are modified to include the fraction of coverage at each chromosome (even if it is 0), filter away the whole genome coverage and duplicates of chromosomes, and adding a column with the respective receptor. 
...
Merged_Chromosome_Coverage <- rbind(Simple_ERa,Simple_ERb) # Merge ERa and ERb to plot both receptors coverage in same graph.

ggplot(Merged_Chromosome_Coverage) + geom_bar(aes(x=Chromosome, y=Fraction_of_Coverage, fill=Receptor), stat = "identity", position="dodge") # plot the fraction of coverage at each chromosome for both ERa and ERb
...
bedtools intersect -a ERa_hg18.bed -b ERb_hg18.bed -c > ERa_count.bed # each ERa site is counted for the number of ERb sites it overlaps (1 or 0)  

bedtools intersect -a ERb_hg18.bed -b ERa_hg18.bed -c > ERb_count.bed # each ERb site is counted for the number of ERa sites it overlaps (1 or 0)  
...
wc -l ERa_overlap_ERb
wc -l ERa_NOT_overlap_ERb
wc -l ERb_overlap_ERa
wc -l ERb_NOT_overlap_ERa

```

We use R to make the piecharts.
...
#Draw Venn Diagram
draw.pairwise.venn(area1=ERa_sites,area2 = ERb_sites,cross.area = Overlapping_Sites, category = c("ERa", "ERb"))

```

The piechart indicate that larger fraction of ERb site overlab ERa sites, compared to ERa sites overlaping ERb sites.   
