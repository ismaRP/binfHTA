...
Pseudocounts are useful because they do not affect large numbers but help smooth the data when there are small values. In this particular case, the advantage of a pseudocount 1 are:

1. The use of a pseudocount of 1 followed by a log2 transformation has the advantage of avoiding the $-\infty$ that would be produced by log2(0).

2. On the other hand, by using a pseudocount of 1, 0 values will remain 0 after the log2 transformation.
...
When using 'scale='row'' the difference between the isoforms and sample clusters are visually clearer,
because without scaling, genes with high variance, hide the changes in genes with less variance but with a significant difference between the two conditions. When scaling, all the variances across rows are equal.
...
OurDesignMatrix <- data.frame(sampleID = c('KO1','KO2','KO3','WT1','WT2','WT3'),
...
#attempt with extractTopSwitches

TopSwitches <- extractTopSwitches(switchAnalyzeRlist = My.SwitchList)

knitr::kable(TopSwitches[,c(2,3,6,8)],digits=100, caption="2.3 \n Sorted genes of lowest gene switch q-value")
...
gene.names <- pull(TopSwitches, gene_name)
...
switchPlotTopSwitches(My.SwitchList)
